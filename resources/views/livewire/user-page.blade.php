...
<div class="container text-center mx-auto tag">
	<div class="row px-2 py-4 mx-auto">
        ...
        @foreach($tags as $tag)
            ...
            @foreach($tag->articles as $tagged) 
                wire:click="getTag({{ $tagged->id }})"
            @endforeach 
            type="button" class="btn btn-light shadow lead my-2 mx-1">{{$tag->tag}}</button></div>
        @endforeach
	</div>
	<input wire:model="search" name="search" class="form-control mr-sm-2 shadow" type="text" placeholder="Search" aria-label="Search" style="width: 50%;">
</div>
<br>
